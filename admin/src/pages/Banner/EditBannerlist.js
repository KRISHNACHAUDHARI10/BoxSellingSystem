...
const EditBannerlist = () => {
    ...
    const loadBannerData = async () => {
      ...
      try {
        ...
        if (response && response.data) {
          ...
          // Set images with full URLs
          if (banner.images && Array.isArray(banner.images)) {
            const fullImageUrls = banner.images.map((url) =>
              getFullImageUrl(url)
            );
        ...
        context.setAlertBox({
          open: true,
          error: true,
          msg: error.message || "Failed to load banner data!",
        });
        navigate("/banners/list");
      } finally {
  ...
  useEffect(() => {
    ...
    loadCategories();
  }, [context.catData, context.fetchCategory, context.setAlertBox]); // Added dependencies

  // Load subcategories when banner data is loaded and categories are available
  useEffect(() => {
    const loadSubCategories = async () => {
      if (
        ...
        try {
          console.log("Loading subcategories for category:", bannerData.catId);

          let response;
          try {
            response = await fetchDataFromApi(
              `/api/subCategory?categoryId=${bannerData.catId}`
            );
  ...
  const updateBanner = async (e) => {
    ...
    try {
      ...
      const response = await putData(`/api/banners/${id}`, payload);
      ...
      if (response && response.success !== false) {
        context.setAlertBox({
          open: true,
          error: false,
          msg: response.message || "Banner updated successfully!",
        });
        ...
        throw new Error(
          response?.message || response?.error || "Failed to update banner"
        );
      }
    } catch (err) {
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-3 mb-0">Loading banner data...</p>
        </div>
      </div>
    );
  }

  // Show error state if banner data couldn't be loaded
  if (!bannerData) {
    return (
      <div
      ...
      >
        <div>
          <h5 className="text-danger">Banner Not Found</h5>
          <p className="text-muted">
            The banner you're trying to edit could not be found.
          </p>
          <Button variant="contained" onClick={() => navigate("/banners/list")}>
    ...
    <>
      ...
      <form className="form" onSubmit={updateBanner}>
        <div className="card p-4 mt-4">
          ...
          {process.env.NODE_ENV === "development" && (
            <div className="mb-3 p-2 bg-light rounded">
              <small>
                ...
                <br />
                Banner ID: {id} | Categories loaded:{" "}
                {context.catData?.length || 0} | Categories loading:{" "}
                {categoriesLoading ? "Yes" : "No"} | Subcategories:{" "}
                {subCategories.length} | Selected Cat: {selectedCategory} |
                Selected SubCat: {selectedSubCategory} | Images:{" "}
                {previews.length}
                <br />
                <strong>Original Banner Data:</strong>
                <br />
                Cat ID: {bannerData?.catId} | Cat Name: {bannerData?.catName} |
                SubCat ID: {bannerData?.subCatId} | SubCat Name:{" "}
                {bannerData?.subCatName}
              </small>
            </div>
          )}
          ...
          <h5 className="mb-4">Banner Images</h5>
          ...
          <div className="mt-4">
            ...
            >
              {isLoading ? (
                ...
                `Update Banner (${previews.length} image${
                  previews.length !== 1 ? "s" : ""
                })`
              )}
            </Button>
